...
class DocumentChunker:
    ...
    def chunk_document(self, 
                      text: str, 
                      source_file: str,
                      strategy: str = "sliding_window") -> List[TextChunk]:
        """
        ...
        meaningful_chunks = []
        for chunk in chunks:
            if (len(chunk.content) >= self.min_chunk_size and 
                self.cleaner.is_meaningful_text(chunk.content)):
    ...
    def _sentence_based_chunking(self, text: str, source_file: str) -> List[TextChunk]:
        ...
        for sentence in sentences:
            ...
            if (current_length + sentence_length > self.chunk_size and 
                current_chunk and current_length >= self.min_chunk_size):
    ...
    def _paragraph_based_chunking(self, text: str, source_file: str) -> List[TextChunk]:
        ...
        for paragraph in paragraphs:
            ...
            if (current_length + paragraph_length > self.chunk_size and 
                current_chunk and current_length >= self.min_chunk_size):
        ...
        for i in range(target_pos, min(target_pos + search_range, len(text))):
            if text[i] in '.!?' and (i + 1 >= len(text) or text[i + 1].isspace()):
                ...
                if (next_char_pos >= len(text) or 
                    text[next_char_pos].isupper() or 
                    text[next_char_pos] in 'ČĆŽŠĐ'):
                    return i + 1
...
def chunk_croatian_document(text: str, 
                          source_file: str,
                          chunk_size: int = 512,
                          overlap: int = 50,
                          strategy: str = "sliding_window") -> List[TextChunk]:
