...
from ..utils.config_loader import (get_language_specific_config,
                                   get_search_config, get_storage_config)
    ...
    def __post_init__(self):
        ...
        if self.language is None:
            try:
                ...
                logger = logging.getLogger(__name__)
                logger.warning(
                    f"Failed to load language code, using fallback 'hr': {e}"
                )
        ...
        try:
            ...
            if self.config.persist:
                self.client = chromadb.PersistentClient(
                    path=self.config.db_path, settings=settings
                )
            else:
                self.client = chromadb.EphemeralClient(settings=settings)

            self.logger.info(
                f"ChromaDB client initialized with persist={self.config.persist}"
            )
    ...
    def query_similar(
        ...
        if n_results is None:
            try:
                ...
                self.logger.warning(
                    "Check your config/vectordb.toml and config/search.toml files"
                )
    ...
    def get_collection_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "persist_directory": (
                    self.config.db_path if self.config.persist else None
                ),
            }
