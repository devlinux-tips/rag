...
class RetrievalConfig:
    ...
    def from_config(
        ...
        if config_dict:
            ...
            # Get retrieval config through provider
            full_config = config_provider.load_config("config")
            config = full_config["retrieval"]
        ...
        return cls(
            ...
            timeout_seconds=config["max_retries"],  # Reuse as timeout
        )
...
class IntelligentRetriever:
        ...
        try:
            ...
            if processed_query.confidence < 0.1:
                return self._create_low_confidence_result(
                    query, processed_query, start_time
                )
            ...
            result = RetrievalResult(
                ...
                metadata={
                    "search_method": (
                        search_response.method.value if search_response else None
                    ),
                    "original_results": (
                        search_response.total_results if search_response else 0
                    ),
    ...
    def _simple_search(
        ...
        if self.config.adaptive_top_k:
            if processed_query.query_type == QueryType.SUMMARIZATION:
                top_k = min(
                    15, self.config.max_results + 5
                )  # More results for summaries
    ...
    def _calculate_retrieval_confidence(
        ...
        if documents:
            avg_score = sum(doc["relevance_score"] for doc in documents) / len(
                documents
            )
            if avg_score > 0.7:
                confidence += 0.2
    ...
    def _create_low_confidence_result(
        ...
        return RetrievalResult(
            ...
            metadata={
                ...
                "suggestions": self.query_processor.suggest_query_improvements(
                    processed_query
                ),
            },
        )
    ...
    def _create_error_result(
        self, query: str, error: str, start_time: float
    ) -> RetrievalResult:
    ...
    def _update_stats(
        self, strategy: RetrievalStrategy, time_taken: float, success: bool
    ):
        ...
        self._query_stats["average_time"] = (
            current_avg * (total - 1) + time_taken
        ) / total
    ...
    def suggest_retrieval_improvements(self, result: RetrievalResult) -> List[str]:
        ...
        if result.result_count == 0:
            ...
            suggestions.append(
                "Upit je možda previše specifičan - probajte generalniji pristup"
            )
        ...
        if result.strategy_used == RetrievalStrategy.SIMPLE:
            if result.processed_query.query_type == QueryType.COMPARISON:
                suggestions.append(
                    "Za usporedbe koristite hibridnu strategiju pretrage"
                )
