...
class HybridRetriever:
    ...
    def search(
        ...
        for dense_result in dense_results:
            ...
            hybrid_score = (
                self.dense_weight * dense_score + self.sparse_weight * bm25_score
            )
    ...
    def explain_scores(self, results: List[HybridResult]) -> str:
        ...
        explanation += f"Dense weight: {self.dense_weight:.1f}, BM25 weight: {self.sparse_weight:.1f}\n\n"

        for i, result in enumerate(results, 1):
            explanation += f"{i}. Score: {result.score:.3f}\n"
            explanation += (
                f"   Dense: {result.dense_score:.3f}, BM25: {result.bm25_score:.3f}\n"
            )
