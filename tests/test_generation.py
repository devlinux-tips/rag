...
class TestOllamaClient:
    ...
    def config(self):
        ...
        return OllamaConfig(base_url="http://test:11434", model="test-model", temperature=0.5)
    ...
    async def test_async_generation_error(self, client):
        ...
        request = GenerationRequest(
            prompt="Test prompt", context=["Context chunk"], query="Test query"
        )
...
class TestPromptTemplates:
    ...
    def test_cultural_context_template(self):
        ...
        assert (
            "cultural context" in template.user_template.lower()
            or "kulturni kontekst" in template.user_template.lower()
        )
    ...
    def test_query_type_selection(self):
        ...
        summary_queries = ["Sažmi povijest Hrvatske", "Ukratko o hrvatskim gradovima"]
    ...
    def test_context_length_limiting(self):
        ...
        system_prompt, user_prompt = builder.build_prompt(
            "Test query", long_context, max_context_length=500
        )
...
class TestFactoryFunctions:
    ...
    def test_create_ollama_client(self):
        ...
        client = create_ollama_client(
            model="custom-model", base_url="http://custom:8080", temperature=0.3
        )
...
class TestIntegration:
    ...
    def mock_client(self):
        ...
        client.generate_response.return_value = (
            "Zagreb je glavni grad Hrvatske sa oko 800.000 stanovnika."
        )
        return client
    ...
    def test_cultural_query_integration(self):
        ...
        assert "kultura" in template.system_prompt.lower()
        assert (
            "traditional" in template.system_prompt.lower()
            or "tradicij" in template.system_prompt.lower()
        )
