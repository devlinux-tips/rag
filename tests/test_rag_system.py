...
from src.pipeline.config import RAGConfig, ProcessingConfig, OllamaConfig
...
class TestRAGResponse:
    ...
    def test_rag_response_creation(self):
        ...
        response = RAGResponse(
            ...
            query="Što je Zagreb?",
            retrieved_chunks=[
                {"content": "Zagreb info", "similarity_score": 0.9}
            ],
            confidence=0.85,
            generation_time=2.5,
            retrieval_time=1.0,
    ...
    def test_high_confidence_property(self):
        ...
        high_response = RAGResponse(
            answer="Test", query="Test", retrieved_chunks=[], confidence=0.85,
            generation_time=1.0, retrieval_time=1.0, total_time=2.0,
            sources=[], metadata={}
        )
        ...
        low_response = RAGResponse(
            answer="Test", query="Test", retrieved_chunks=[], confidence=0.75,
            generation_time=1.0, retrieval_time=1.0, total_time=2.0,
            sources=[], metadata={}
        )
...
class TestCroatianRAGSystem:
    ...
    async def test_vectordb_initialization(self, rag_system):
        ...
        with patch('src.pipeline.rag_system.EmbeddingModel') as mock_embedding, \
    ...
    async def test_generation_initialization(self, rag_system):
        ...
        with patch('src.pipeline.rag_system.OllamaClient') as mock_client, \
    ...
    async def test_add_documents_success(self, rag_system):
        ...
        rag_system._text_cleaner.clean_text = Mock(
            return_value="zagreb je glavni grad hrvatske"
        )
    ...
    async def test_query_success(self, rag_system):
        ...
        rag_system._query_processor.process_query = Mock(
            return_value=mock_processed_query
        )
        ...
        rag_system._retriever.retrieve_async = AsyncMock(
        ...
        rag_system._response_parser.parse_response = Mock(
    ...
    async def test_health_check(self, rag_system):
        ...
        rag_system._ollama_client.get_available_models = Mock(
...
class TestRAGSystemIntegration:
    ...
    async def test_create_rag_system_factory(self):
        ...
        with patch.multiple(
            'src.pipeline.rag_system.CroatianRAGSystem',
