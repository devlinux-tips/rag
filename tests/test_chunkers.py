    ...
    def test_sentence_based_chunking_respect_sentences(self):
        ...
        for chunk in chunks:
            ...
            if content:
                ...
                assert content.endswith('.') or content.endswith('!') or content.endswith('?') or len(content) >= self.chunker.chunk_size * 0.8
...
Treći paragraf."""
        ...
        paragraph_indicators = sum(1 for chunk in chunks if '\n\n' in chunk.content or len(chunk.content.strip()) > 10)
