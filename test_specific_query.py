...
async def test_specific_query():
    ...
    storage_config = StorageConfig(
        collection_name="specific_query_test", db_path="./temp_chroma_db"
    )
    ...
    for doc_file in documents[:3]:  # Process first 3 documents
        try:
            ...
            if "2025" in text and (
                "EUR" in text or "euro" in text.lower() or "eur" in text.lower()
            ):
    ...
    results = storage.query_similar(query_embeddings=[query_embedding.tolist()], n_results=5)
