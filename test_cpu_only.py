...
async def test_cpu_pipeline():
    ...
    try:
        ...
        storage_config = StorageConfig(
            collection_name="test_collection", db_path="./temp_chroma_db"
        )
        ...
        for i, chunk in enumerate(chunks[:3]):  # Only first 3 chunks
            ...
            metadatas.append({"chunk_id": chunk.chunk_id, "source": "sample.txt", "language": "hr"})
        ...
        if chunks:
            ...
            if results and "documents" in results and results["documents"][0]:
                ...
                for i, (doc, distance) in enumerate(
                    zip(results["documents"][0][:1], results["distances"][0][:1])
                ):
        ...
        for query in test_queries:
            ...
            print(
                f"✅ Query: '{query}' -> Type: {processed.query_type}, Confidence: {processed.confidence:.3f}"
            )
